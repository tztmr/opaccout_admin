...
- OP到期时间取卡密最后一段 10 位 Unix 秒级时间戳，再固定增加 90 天；公开短 OP 解析会先请求 `get_simple_userinfo`，QQ 侧仍有效时不以本地 90 天展示到期时间为唯一拦截依据；API 启动时会按解密后的 `auth_time + 90 天` 回填已有账号的 `opExpiresAt`
...
QQ OP 查询由 API 服务器直接访问 `graph.qq.com`，不使用 SOCKS 代理。
