...
function parseAccountKind(value: string | null): AccountKind {
...
export function ImportsPage() {
  ...
  const accountKind = parseAccountKind(searchParams.get("accountKind"));
  ...
  return <section>
    ...
    <div className="import-grid">
      ...
      <form className="upload-card paste-card" onSubmit={handlePasteSubmit}>
        ...
        <h2>文本快捷导入</h2>
        <p>支持多行粘贴，各字段用 <code>----</code> 分隔。{accountKind === "email" && <>邮箱号导入必须提供<strong>邮箱</strong>列。</>}<br/>若每行仅有 <code>抖音号----注册时间</code>，则自动进入<strong>覆盖时间模式</strong>。</p>
        <textarea 
    ...
    <div className="panel">
      ...
      <div className="table-scroll"><table><thead><tr><th>文件名</th><th>账号类型</th><th>状态</th><th>总数</th><th>新增</th><th>更新</th><th>跳过</th><th>失败</th><th>失败原因</th><th>提交时间</th></tr></thead>
      <tbody>{jobs.data?.length ? jobs.data.map((job) => {
        ...
        return <tr key={job._id}>
          <td>{job.fileName}</td>
          <td>{accountKindLabels[job.accountKind ?? "google"]}</td>
      ...
      }) : <tr><td className="empty" colSpan={10}>{jobs.isLoading ? "正在加载…" : "暂无导入任务"}</td></tr>}</tbody></table></div>
    </div>
  </section>;
}
