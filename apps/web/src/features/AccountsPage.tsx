...
export function AccountsPage() {
  ...
  return <section>
    ...
    <div className="table-panel">
      ...
      </tr>):<tr><td colSpan={13} className="empty">{search||saleStatus||accountStatus||owner||registeredFrom||registeredTo?"当前筛选无结果":"尚无账号数据"}</td></tr>}</tbody></table></div>
      <div className="pager">
...
function BatchUpdateDialog({state,owners,close,submit,setValue}:{state:BatchDialogState;owners:string[];close():void;submit():void;setValue(value:string):void}) {
  const title = state.type==="status" ? "修改售卖状态" : "修改归属人";
  const confirmDisabled = state.type==="owner" ? !state.value.trim() : !state.value;
