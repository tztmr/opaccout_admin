...
const blank = { douyinId:"", registeredAt:new Date().toISOString().slice(0,10), opName:"", opSecret:"", owner:"", saleStatus:"unsold", remark:"" };
...
export function AccountsPage() {
  ...
  const mutate = useMutation({
    ...
    onSuccess:()=>{setDrawer(null);setMessage("保存成功");void client.invalidateQueries({queryKey:["accounts"]})},
    onError:(error)=>setMessage(error instanceof Error?error.message:"保存失败")
  });
  const remove = async(id:string)=>{if(!confirm("确定删除这条账号吗？"))return;await api(`/api/accounts/${id}`,{method:"DELETE"});void client.invalidateQueries({queryKey:["accounts"]})};
  ...
  const runBatch = async(action:"status"|"owner"|"recheck"|"delete")=>{
    ...
    setSelected(new Set());void client.invalidateQueries({queryKey:["accounts"]});
  };
  ...
  if(selected.size)exportParams.set("ids",[...selected].join(","));
  else for(const key of ["keyword","saleStatus","accountStatus","registeredFrom","registeredTo"]){const value=urlParams.get(key);if(value)exportParams.set(key,value)}
  return <section>
    ...
    <div className="table-panel">
      <div className="toolbar">
        ...
        {(search||saleStatus||accountStatus||registeredFrom||registeredTo)&&<button onClick={()=>{setKeyword("");setUrlParams({}, {replace:true})}}>清空</button>}
      ...
      </tr>):<tr><td colSpan={12} className="empty">{search||saleStatus||accountStatus||registeredFrom||registeredTo?"当前筛选无结果":"尚无账号数据"}</td></tr>}</tbody></table></div>
    ...
    {drawer&&<AccountDrawer state={drawer} busy={mutate.isPending} close={()=>setDrawer(null)} submit={value=>mutate.mutate(drawer.id?{id:drawer.id,value}:{value})}/>}
  </section>;
}

function AccountDrawer({state,busy,close,submit}:{state:{mode:"create"|"edit";value:AccountFormValue};busy:boolean;close():void;submit(v:AccountSubmitValue):void}) {
  ...
  return <div className="overlay" onMouseDown={e=>{if(e.target===e.currentTarget)close()}}><form className="drawer" onSubmit={onSubmit}><header><div><h2>{state.mode==="create"?"新增账号":"编辑账号"}</h2><p>派生字段由服务端自动计算</p></div><button type="button" className="icon-button" onClick={close}><X/></button></header>
    ...
    <label>OP卡密<input name="opSecret" defaultValue="" required={state.mode==="create"} placeholder={state.mode==="edit"?"不修改请留空":"末段必须为10位时间戳"}/></label><label>归属人<input name="owner" defaultValue={state.value.owner} required/></label>
