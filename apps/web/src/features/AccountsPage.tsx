...
export function AccountsPage({ accountKind }: { accountKind: AccountKind }) {
  ...
  return <section>
    ...
    <div className="table-panel">
      ...
      <tbody>{query.isLoading?<tr><td colSpan={columns.length + 3} className="empty">正在加载…</td></tr>:query.isError?<tr><td colSpan={columns.length + 3} className="empty">{`搜索失败：${queryErrorMessage}`}</td></tr>:data?.items.length?data.items.map((row,index)=><tr key={row._id}>
